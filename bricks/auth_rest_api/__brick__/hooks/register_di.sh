...
# Register Auth REST API Dependencies
...
if [ -f "$DI_FILE" ]; then
    # Add imports
    grep -q "import.*auth.*rest_auth_remote_data_source" "$DI_FILE" || {
        sed -i '' '/^import.*service_locator/a\
\
// Auth REST API Imports\
import '\''../../features/auth/data/remote/data_sources/rest_auth_remote_data_source.dart'\'';\
import '\''../../features/auth/data/remote/data_sources/rest_auth_remote_data_source_impl.dart'\'';\
...
' "$DI_FILE"
    ...
    # Add registrations
    grep -q "registerLazySingleton<RestAuthRemoteDataSource>" "$DI_FILE" || {
        sed -i '' '/void _registerDataSources()/,/^  }$/{
            /^  }$/ i\
\
    // Auth REST API Data Sources\
    registerLazySingleton<RestAuthRemoteDataSource>(\
...
\
    ...
    echo "✅ Auth REST API DI registration completed"
else
