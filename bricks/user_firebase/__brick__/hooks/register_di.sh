...
# Register User Firebase Dependencies
...
if [ -f "$DI_FILE" ]; then
    # Add imports
    grep -q "import.*user.*firebase_user_remote_data_source" "$DI_FILE" || {
        sed -i '' '/^import.*service_locator/a\
\
// User Firebase Imports\
import '\''../../features/user/data/remote/data_sources/firebase_user_remote_data_source.dart'\'';\
import '\''../../features/user/data/remote/data_sources/firebase_user_remote_data_source_impl.dart'\'';\
...
' "$DI_FILE"
    ...
    # Add registrations
    grep -q "getit.registerLazySingleton<FirebaseUserRemoteDataSource>" "$DI_FILE" || {
        sed -i '' '/void _registerDataSources()/,/^  }$/{
            /^  }$/ i\
\
    // User Firebase Data Sources\
    getit.registerLazySingleton<FirebaseUserRemoteDataSource>(\
...
\
    ...
    echo "✅ User Firebase DI registration completed"
else
