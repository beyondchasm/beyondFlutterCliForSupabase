...
# Register User REST API Dependencies
...
if [ -f "$DI_FILE" ]; then
    # Add imports
    grep -q "import.*user.*rest_user_remote_data_source" "$DI_FILE" || {
        sed -i '' '/^import.*service_locator/a\
\
// User REST API Imports\
import '\''../../features/user/data/remote/data_sources/rest_user_remote_data_source.dart'\'';\
import '\''../../features/user/data/remote/data_sources/rest_user_remote_data_source_impl.dart'\'';\
...
\
    ...
    echo "✅ User REST API DI registration completed"
else
